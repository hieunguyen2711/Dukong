...
export function isCourseOffered(courseId, semester, year) {
  const offerings = loadOfferingData();
...
export function getAvailableCoursesForSemester(semester, year) {
  const offerings = loadOfferingData();
  const availableCourses = [];

  for (const [courseId, offeringCode] of Object.entries(offerings)) {
    if (isCourseOffered(courseId, semester, year)) {
...
export function validateCourseSelection(courseId, semester, year) {
  const isOffered = isCourseOffered(courseId, semester, year);
...
export function getOfferingDescription(courseId) {
  const offerings = loadOfferingData();
  const offeringCode = offerings[courseId];
